...
# local modules - ensure local package path is on sys.path so script runs directly
import sys
...
for candidate in ("Segoe UI", "Helvetica", "Arial", "Sans Serif", "Sans"):
...
# ================= BACKGROUND (with fallback and asset lookup) =================
bg_candidates = [
    ROOT / "assets" / "background.png",
    ROOT / "assets" / "bg.png",
    ROOT / "image" / "bg.png",
    ROOT / "image" / "Screenshot 2025-12-16 114913.png",
]
...
try:
    if bg_path is not None:
        ...
        raise FileNotFoundError("No background file found in candidates")
...
def _show_confetti(duration=1200, count=40):
    try:
        ...
        w = c.winfo_reqwidth() or 360
        h = c.winfo_reqheight() or 140
        for i in range(count):
            x = _r.randint(10, max(10, w-10))
            y = _r.randint(10, max(10, h-10))
            r = _r.randint(3, 8)
...
def start_level():
    ...
    entry = Entry(panel, font=(BASE_FONT, 18),
...
def check_guess(entry):
    ...
    raw = entry.get().strip()
    # clear input immediately after pressing Enter/Submit and keep focus
    try:
        entry.delete(0, END)
    except Exception:
        pass
...
try:
    ...
    logging.exception("Unhandled exception in GUI: %s", e)
    try:
